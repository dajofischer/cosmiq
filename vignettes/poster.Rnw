%\VignetteIndexEntry{13th Annual Conference of the Metabolomics Society (in portrait)}
...
The bioconductor package has been 
...
%TODO:fig
<<label=image, keep.source = TRUE, fig = TRUE, include = FALSE>>=
eicmat <- 
...
@
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/cosmiq-image}
}
...
idx <- order(rowSums(peaktable[,8:19]), 
	decreasing=TRUE)
@

<<xtable1, results=tex, echo=TRUE>>=
library(xtable)
xtab <- peaktable[idx[1:20], ]
print.xtable(xtable(xtab, caption="Result", 
	label="Table:xtable1", 
